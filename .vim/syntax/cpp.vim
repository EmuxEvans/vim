   " Vim syntax file
   " Language: C++ special highlighting for STL, functions, digits and operations
   " Author: Weiwei
   " Last Change:  2008 Aug 6
   ...
   syn keyword cppType     static inline virtual explicit export bool wchar_t int void float double const
   ...
   if version >= 508 || !exists("did_cpp_syntax_inits")
     if version < 508
       ...
       ghlight All Function
     else
       command -nargs=+ HiLink hi def link <args>
     endif
     HiLink cppAccess      cppStatement
     HiLink cppCast        cppStatement
     HiLink cppExceptions      ErrorMsg
     hi cppOperator        guifg=white
     HiLink cppStatement       Statement
     HiLink cppSTL         Identifier
     HiLink cppNCBI        Todo
     HiLink cppType        Title
     HiLink cppStorageClass    Directory
     HiLink cppStructure       SpecialKey
     HiLink cppNumber      Constant
     HiLink cppBoolean     NonText
     delcommand HiLink
   endif
   ...
   hi cppFunction cterm=bold guifg=Red
   "========================================================
   ...
   " Math Operator
   hi cppMathOperator            guifg=Blue
   hi cppPointerOperator         guifg=Green
   hi cppLogicalOperator         guifg=White
   hi cppBinaryOperator          guifg=White
   hi cppBinaryOperatorError     guifg=Red
   hi cppLogicalOperator         guifg=White
   hi cppLogicalOperatorError    guifg=Red
   "number
   syn match cppNumber display "[0-9][0-9]*"
   hi cppNumber guifg=Yellow
   "comment
   syn match cppComment "//[^`]*"
   hi cppComment guifg=White
