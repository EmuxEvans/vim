...
"exe "amenu <silent> ".s:Comments.'.file\ description\ \(header\)<Tab>\\cfdh  :call C_InsertTemplate("comment.file-description-header")<CR>'
	exe "amenu <silent> ".s:Comments.'.file\ description\ \(impl\.\)<Tab>\\cfdi  :call C_InsertTemplateWrapper()<CR>'
	exe "amenu <silent> ".s:Comments.'.file\ description\ \(header\)<Tab>\\cfdh  :call C_InsertTemplateWrapper()<CR>'
...
"
	...
	"Added by emux for cgi
	exe "amenu <silent> ".s:Idioms.'.web<Tab>\\iw																:call C_InsertTemplate("idioms.web")<CR>'
	exe "imenu <silent> ".s:Idioms.'.web<Tab>\\iw													 <Esc>:call C_InsertTemplate("idioms.web")<CR>'
	"
...
function! C_InsertTemplate ( key, ... )
  ...
  "Modified by emux
  "-----------------
  "ReRead the template to get right USER MACROs such as |AUTHOR|, |COPYRIGHT|
  "etc. for <leader>cfdi or <leader>cfdh command
	...
	while match( template, s:C_NonExpansionRegex ) != -1
		...
		if match[1] != ''
			...
			if has_key( s:C_Macro, macroname )
				...
				"emux
				"Free the C_Macro[maconame]
				let s:C_Macro[macroname] = ''
			else
				"
