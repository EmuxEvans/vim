...
cp build.xml tests/report/junit
...
leakXml="${leakDir}/valgrind.xml"
leakPng="${leakDir}/leakcheck.png"

if [ "x$Os" == "xlinux"];then
  cp config.status.bck config.status && ./config.status
fi
make clean && make && valgrind --xml=yes --xml-file=${leakXml} --log-file=${leakLog} --leak-check=full --show-reachable=yes --track-origins=yes ${testProg}
cd ${leakDir}
valgrind2xunit.py
cd -
